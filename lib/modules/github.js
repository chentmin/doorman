...
var checkOrganization = function(auth) {
  ...
  log.notice("User rejected because they aren't in the organization list: " + requiredOrganization.join(", "));

  return false;
};

var checkEmail = function(auth) {
  var email = auth.github.user.emails;

  for (var i = 0; i < email.length; i++){
    if (requiredEmail.indexOf(email[i])){
      return true
    }
  }
...
exports.setup = function(everyauth) {
  everyauth.github
    ...
    .findOrCreateUser( function (sess, accessToken, accessTokenExtra, ghUser) {
      ...
      this.oauth.get(this.apiHost() + '/user/emails', accessToken, function (err, data) {
        if (err) return p.fail(err);
        ghUser.emails = new Array();
        var emails = JSON.parse(data);
        for(var i = 0; i < emails.length; i++){
          ghUser.emails.push(emails[i].email);
        }
        p.fulfill(ghUser);
      }
      );
  ...
  if(requiredOrganization) {
    log.notice("Registered Github authentication for members of organizations: " + requiredOrganization.join(", "));
  }
  if(requiredEmail) {
    log.notice("Registered Github authentication for users: " + requiredEmail.join(", "));
  }
};
